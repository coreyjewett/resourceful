...
Resource.truncate = function (callback) {
  return this._request("truncate", callback);
};
...
function cloningMixin(target) {
  var objs = Array.prototype.slice.call(arguments, 1);
  objs.forEach(function (o) {
    Object.keys(o).forEach(function (k) {
      if (!o.__lookupGetter__(k)) {
        target[k] = Object.create(o[k]);
      }
    });
  });
  return target;
...
Resource.mixin = function(resourcez) {
  var target = this;
  var objs = Array.prototype.slice.call(arguments, 0);
  objs.forEach(function (resource) {
    if (typeof resource === "string") {
      resource = resourceful.resources[common.capitalize(resource)];
      if (!resource) {
        throw new(TypeError)("argument must be a valid resource name");
      }
    } else if (resource.__proto__ !== resourceful.Resource) {
      console.log(resource);
      throw new Error("Can't mixin a non-Resource");
    }

    target.schema.properties = cloningMixin(target.schema.properties, resource.schema.properties);
  });
  return this;
}
...
Resource.prototype.truncate = function () {};
